...
export function DependencyPanel({ dependencies, isUpdating, onResolve }: DependencyPanelProps) {
  ...
  return (
    <section aria-labelledby="dependencies-heading" className="rounded-radius-md border border-neutral-200 bg-neutral-50">
      <div className="flex items-start justify-between gap-4 border-b border-neutral-200 p-4">
        <div>
          <h2 id="dependencies-heading" className="text-h2 font-semibold text-neutral-900">Next action: resolve the blockers</h2>
          <p className="mt-1 text-small text-neutral-600">Named dependencies from the investigation record. Resolve the outstanding item before returning to the worklist.</p>
        </div>
        <CircleAlert className="h-5 w-5 shrink-0 text-status-warning" aria-hidden="true" />
      </div>
      <ul className="divide-y divide-neutral-200" aria-label="Case dependencies">
            ...
            <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
              <div className="min-w-0">
                ...
                <div className="mt-2 flex flex-wrap items-center gap-x-4 gap-y-2 text-small text-neutral-600">
                  <span className="inline-flex items-center gap-1"><UserRound className="h-4 w-4" aria-hidden="true" />{dependency.assigned_to ?? 'Owner not recorded'}</span>
                  <span className="tabular-nums">{dependency.days_stale} days waiting</span>
                  <span>Priority not recorded</span>
                </div>
              </div>
